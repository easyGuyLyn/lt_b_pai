...
public class BJKL8_X5_Fragment extends BaseFragment {
    ...
    private void initData() {
        ThreadUtils.newThread(new Runnable() {
            @Override
            public void run() {
                mList = XYBDataUtils.initBJKL8ChooseData(5);
                ...
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mTvPlayTypeName.setText(mList.get(0).getPlayTypeName() + "(中5@" + add[0] + " " + "中4@" + add[1] + " " + "中3@" + add[2] + "）");
    ...
    public void randomSelected(String id) {
        if (!isHidden()) {
            ...
            int[] randomChild = XYBDataUtils.getRandomList(5, 0, mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).size());
