...
public class QT_KD_Fragment extends BaseFragment {
    ...
    private void initData() {
        ThreadUtils.newThread(new Runnable() {
            ...
            public void run() {
                if (mList.isEmpty()) {
                    mList = QTBDataUtil.initKDData();
                }
                for (PlayTypeBean.PlayBean playBean : mList) {
                    mListLinkedHashMap.put(playBean.getPlayTypeName(), QTBDataUtil.init_KD_0_9_Data(playBean.getPlayTypeName()));
                }
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            int randomIndex = RandomUtils.randInt(mListLinkedHashMap.get(getString(R.string.KD)).size());
                            mListLinkedHashMap.get(getString(R.string.KD)).get(randomIndex).setSelected(true);
                            selectedBean.add(mListLinkedHashMap.get(getString(R.string.KD)).get(randomIndex));
                            ((QTBActivity) getActivity()).onTouZhuView(mListLinkedHashMap.get(getString(R.string.KD)).get(randomIndex));
                            mAdapter.notifyDataSetChanged();
                        }
                    });
