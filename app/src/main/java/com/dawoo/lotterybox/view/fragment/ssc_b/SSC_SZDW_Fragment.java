...
public class SSC_SZDW_Fragment extends BaseFragment {
    ...
    private String playName = BoxApplication.getContext().getString(R.string.wqb);

    public SSC_SZDW_Fragment() {
    }
    ...
    private void initViews() {
        mList = SSCBDataUtil.initSZDWData();
        initTab();
    ...
    private void initData() {
        ...
        for (PlayTypeBean.PlayBean playBean : mList) {
            ...
            for (String childType : childTypes) {
                listLinkedHashMap.put(childType, SSCBDataUtil.init_SZDW_0_9_Data(
        ...
        if (!isHidden()) {
            {
                ThreadUtils.newThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        int randomChild2 = RandomUtils.randInt(mStringLinkedHashMapLinkedHashMap.get(playName).get(playName.charAt(2) + "").size() - 1);
                        mStringLinkedHashMapLinkedHashMap.get(playName).get(playName.charAt(2) + "").get(randomChild2).setSelected(true);
                        PlayDetailBean bean2 = mStringLinkedHashMapLinkedHashMap.get(playName).get(playName.charAt(2) + "").get(randomChild2);

                        selectedBean.add(bean);
                        selectedBean.add(bean1);
                        selectedBean.add(bean2);
                        getActivity().runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                            //    ((SSCBActivity) getActivity()).offTouZhuView();
                                ...
                                ((SSCBActivity) getActivity()).onTouZhuView(bean2);
                            }
                        });
                    }
                });
