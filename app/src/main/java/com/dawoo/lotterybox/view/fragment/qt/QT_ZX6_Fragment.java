...
public class QT_ZX6_Fragment extends BaseFragment {
    ...
    private void initData() {
        ThreadUtils.newThread(new Runnable() {
            @Override
            public void run() {
                mList = QTBDataUtil.initZX6Data();
                for (PlayTypeBean.PlayBean playBean : mList) {
                    mListLinkedHashMap.put(playBean.getPlayTypeName(), QTBDataUtil.init_ZX6_0_9_Data(playBean.getPlayTypeName()));
                }
    ...
    public void randomSelected(String id) {
        if (!isHidden()) {
            ThreadUtils.newThread(new Runnable() {
                ...
                public void run() {
                    if (selectedBean.size() > 0) {
                        mListLinkedHashMap.put(BoxApplication.getContext().getString(R.string.ZXL), Lottery_B_DataUtils.clearSelected(mListLinkedHashMap.get(BoxApplication.getContext().getString(R.string.ZXL))));
                        selectedBean.clear();
                        ((QTBActivity) getActivity()).clearSelected();
                    }
                    int[] selected = LHCUtil.getRandomList(4, 0, mListLinkedHashMap.get(BoxApplication.getContext().getString(R.string.ZXL)).size());
                    getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            for (int i : selected) {
                                mListLinkedHashMap.get(BoxApplication.getContext().getString(R.string.ZXL)).get(i).setSelected(true);
                                selectedBean.add(mListLinkedHashMap.get(BoxApplication.getContext().getString(R.string.ZXL)).get(i));
                                ((QTBActivity) getActivity()).onTouZhuView(mListLinkedHashMap.get(BoxApplication.getContext().getString(R.string.ZXL)).get(i));
                            }
                            mAdapter.notifyDataSetChanged();
                        }
                    });
