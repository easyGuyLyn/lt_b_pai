...
public class BJKL8_X4_Fragment extends BaseFragment {
    ...
    private void initData() {
        mList = XYBDataUtils.initBJKL8ChooseData(4);
        ...
        String add[] = mList.get(0).getScheme().split(",");
        mTvPlayTypeName.setText(mList.get(0).getPlayTypeName() + "(中4@" + add[0] + " " + "中3@" + add[1] + " " + "中2@" + add[2] + "）");
        mTvPlayTypeName.setTextSize(10);
    ...
    public void randomSelected(String id) {
        if (!isHidden()) {
            ...
            int[] randomChild = XYBDataUtils.getRandomList(4, 0, mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).size());
