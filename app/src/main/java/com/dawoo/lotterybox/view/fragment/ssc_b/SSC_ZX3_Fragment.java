...
public class SSC_ZX3_Fragment extends BaseFragment {
    ...
    private String playName = BoxApplication.getContext().getString(R.string.q3zx3);
    private int mCurrentPostion = 0;

    public SSC_ZX3_Fragment() {
    }
    ...
    private void initViews() {
        mList = SSCBDataUtil.initZX3Data();
        initTab();
    }
    ...
    private void initData() {
        ...
        for (PlayTypeBean.PlayBean playBean : mList) {
            ...
            listLinkedHashMap.put("1", SSCBDataUtil.init_ZX3_0_9_Data(
        ...
        if (!isHidden()) {
            ThreadUtils.newThread(new Runnable() {
                ...
                public void run() {
                    ...
                    int[] selecteds = SSCBDataUtil.getRandomList(5, 0, mStringLinkedHashMapLinkedHashMap.get(playName).get("1").size() - 1);
                    getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if (selectedBean.size() > 0) {
