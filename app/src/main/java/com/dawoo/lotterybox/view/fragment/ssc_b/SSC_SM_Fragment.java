...
public class SSC_SM_Fragment extends BaseFragment {
    ...
    private void initViews() {
        mAdapter = new MainAdapter(R.layout.layout_lottery_b_common_item);
    ...
    private void initData() {
        selectedBean.clear();
        mList = SSCBDataUtil.initSMData();
        for (PlayTypeBean.PlayBean playBean : mList) {
            if (!playBean.getPlayTypeName().equals(getString(R.string.zh))) {
                mListLinkedHashMap.put(playBean.getPlayTypeName(), SSCBDataUtil.init_SM_0_9Data(true, playBean.getPlayTypeName()));
            } else {
                mListLinkedHashMap.put(playBean.getPlayTypeName(), SSCBDataUtil.init_SM_0_9Data(false, playBean.getPlayTypeName()));
            }
        }
        ...
        if (!isHidden()) {
            ThreadUtils.newThread(new Runnable() {
                ...
                public void run() {
                    ...
                    int ranomItem = RandomUtils.randInt(mListLinkedHashMap.get(mList.get(randomType).getPlayTypeName()).size() - 1);
                    mListLinkedHashMap.get(mList.get(randomType).getPlayTypeName()).get(ranomItem).setSelected(true);
                    selectedBean.add(mListLinkedHashMap.get(mList.get(randomType).getPlayTypeName()).get(ranomItem));
                    getActivity().runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                        //    ((SSCBActivity) getActivity()).offTouZhuView();
                            ...
                            ((SSCBActivity) getActivity()).onTouZhuView(mListLinkedHashMap.get(mList.get(randomType).getPlayTypeName()).get(ranomItem));
                        }
                    });
                }
            });
