...
public class PK10_GYH_Fragment extends BaseFragment {
    ...
    private void initData() {
        ...
        mList = PK10BDataUtil.initPK10BGYHData();
        for (PlayTypeBean.PlayBean playBean : mList) {
            mListLinkedHashMap.put(playBean.getPlayTypeName(), PK10BDataUtil.init_PK10B_GYH_Data(playBean.getPlayTypeName()));
        }
            ...
            ThreadUtils.newThread(new Runnable() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < mList.size(); i++) {
                        if (i == randomparent) {
                            mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).get(randomparent).setSelected(true);
                            getActivity().runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    if (!selectedBean.isEmpty()) {
                                    ...
                                    int randomItem = RandomUtils.randInt(mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).size() - 1);
                                    selectedBean.add(mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).get(randomItem));
                                    mAdapter.notifyDataSetChanged();
                                    ((PK10BActivity) getActivity()).onTouZhuView(mListLinkedHashMap.get(mList.get(randomparent).getPlayTypeName()).get(randomItem));
